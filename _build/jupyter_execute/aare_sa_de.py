...
glue("aare_city_map_caption", map_caption, display=False)
...
definition_list_items = [ListItem(Paragraph(x, featuredata.p_style), bulletColor=HexColor("#000000")) for x in section_definition]
definition_list = ListFlowable(definition_list_items, bulletType='bullet', start="square", bulletFontSize=6)
...
caption_kwargs = ParagraphStyle(**{"name": "caption_style", "fontSize": 9, "fontName": "Times-Italic", "leftIndent":cm*4, "rightIndent": cm*4})
land_use_caption = Paragraph(figure_caption, caption_kwargs)
...
# ` `
# ```
# {numref}`Abbildung %s: <aare_monthly_results>` {glue:text}`aare_monthly_reults_caption
...
name_list = [ListItem(Paragraph(x, featuredata.p_style), bulletColor=HexColor("#000000")) for x in values]
