...
b_s_a = trb[(trb.code.isin(thecommon))&(trb.water_name_slug == 'bielersee')].groupby('code', as_index=False).fail.sum()
bsee_samps = trb[(trb.water_name_slug == 'bielersee')].loc_date.nunique()
b_s_a['f_r'] = b_s_a.fail/bsee_samps
b_s_a = b_s_a.sort_values(by='f_r', ascending=False)
b_s_a['item'] = b_s_a.code.map(lambda x: code_description_map.loc[x])
b_s_a.set_index('item', inplace=True)
b_s_a['Bielersee'] = b_s_a['f_r']

# the aggregated totals for the river bassin
t_s_a = trb[(trb.code.isin(thecommon))].groupby('code', as_index=False).fail.sum()
t_s_a['rate'] = t_s_a.fail/trb.loc_date.nunique()
t_s_a = t_s_a.sort_values(by='rate', ascending=False)
t_s_a['item'] = t_s_a.code.map(lambda x: code_description_map.loc[x])
t_s_a.set_index('item', inplace=True)
t_s_a['Aare survey area'] = t_s_a['rate']

# repeat for all the survey areas
a_s_a = dfSurveys[(dfSurveys.code.isin(thecommon))].groupby('code', as_index=False).fail.sum()
a_s_a['rate'] = a_s_a.fail/dfSurveys.loc_date.nunique()
a_s_a = a_s_a.sort_values(by='rate', ascending=False)
a_s_a['item'] = a_s_a.code.map(lambda x: code_description_map.loc[x])
a_s_a.set_index('item', inplace=True)
a_s_a['All survey areas'] = a_s_a['rate']

ad_t_ten = pd.concat([mcp, b_s_a['Bielersee'], t_s_a["Aare survey area"], a_s_a["All survey areas"]], axis=1).sort_values(by='Biel/Bienne', ascending=False)
...
m_common_st = biel[biel.code.isin(c_qty)].copy()
...
# the aggregated totals for the lake
b_s_a = trb[(trb.code.isin(thecommon))&(trb.water_name_slug == 'bielersee')].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
b_s_a = b_s_a.groupby('code', as_index=False)[unit_label].median()
b_s_a['item'] = b_s_a.code.map(lambda x: code_description_map.loc[x])
b_s_a.set_index('item', inplace=True)
b_s_a['Bielersee'] = b_s_a[unit_label]

# the aggregated totals for the river bassin
t_s_a = trb[(trb.code.isin(thecommon))].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
t_s_a = t_s_a.groupby('code', as_index=False)[unit_label].median()
t_s_a['item'] = t_s_a.code.map(lambda x: code_description_map.loc[x])
t_s_a.set_index('item', inplace=True)
t_s_a['Aare survey area'] = t_s_a[unit_label]

# repeat for all the survey areas
a_s_a = dfSurveys[dfSurveys.code.isin(thecommon)].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
a_s_a = a_s_a.groupby('code', as_index=False)[unit_label].median()
a_s_a['item'] = a_s_a.code.map(lambda x: code_description_map.loc[x])
a_s_a.set_index('item', inplace=True)
a_s_a['All survey areas'] = a_s_a[unit_label]

ad_t_ten = pd.concat([m_c_p,b_s_a['Bielersee'], t_s_a['Aare survey area'], a_s_a['All survey areas']], axis=1).sort_values(by='Biel/Bienne', ascending=False)
...
bsee = ac
bsee['pt'] = bsee.quantity/biel.quantity.sum()
...
# the aggregated totals for the lake
b_s_a = trb[(trb.code.isin(thecommon))&(trb.water_name_slug == 'bielersee')].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
b_s_a = b_s_a.groupby('code', as_index=False)['quantity'].sum()
b_s_a['pt'] = b_s_a.quantity/trb[(trb.water_name_slug == 'bielersee')].quantity.sum()
b_s_a['item'] = b_s_a.code.map(lambda x: code_description_map.loc[x])
b_s_a.set_index('item', inplace=True)
b_s_a['Bielersee'] = b_s_a['pt']

# the aggregated totals for the river bassin
t_s_a = trb[(trb.code.isin(thecommon))].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
t_s_a = t_s_a.groupby('code', as_index=False)['quantity'].sum()
t_s_a['pt'] = t_s_a.quantity/trb.quantity.sum()
t_s_a['item'] = t_s_a.code.map(lambda x: code_description_map.loc[x])
t_s_a.set_index('item', inplace=True)
t_s_a['Aare survey area'] = t_s_a['pt']

# repeat for all the survey areas
a_s_a = dfSurveys[dfSurveys.code.isin(thecommon)].groupby(['loc_date','code'], as_index=False).agg(agg_pcs_quantity)
a_s_a = a_s_a.groupby('code', as_index=False)['quantity'].sum()
a_s_a['pt'] = a_s_a.quantity/dfSurveys.quantity.sum()
a_s_a['item'] = a_s_a.code.map(lambda x: code_description_map.loc[x])
a_s_a.set_index('item', inplace=True)
a_s_a['All survey areas'] = a_s_a['pt']

ad_t_ten = pd.concat([bsee['Biel/Bienne'], b_s_a['Bielersee'], t_s_a['Aare survey area'], a_s_a['All survey areas']], axis=1).sort_values(by='Biel/Bienne', ascending=False)
