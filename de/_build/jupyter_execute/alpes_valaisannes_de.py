...
# 
# Die häufigsten Objekte sind die zehn häufigsten Objekte nach Anzahl der Funde UND/ODER jedes Objekt, das in mindestens 50% aller Datenerhebungen identifiziert wurde. 
...
t_s_a = wt_data[(wt_data.code.isin(m_common.index))].groupby(["location", "loc_date", "code"], as_index=False).agg(agg_pcs_quantity)
t_s_a_cols = sut.aggregate_to_code(t_s_a, code_description_map,name=level_names[0])
...
a_s_a_cols = sut.aggregate_to_code(a_s_a, code_description_map,name=level_names[1])

# # merge all that
ad_t_ten = pd.concat([m_c_p, t_s_a_cols, a_s_a_cols,], axis=1).sort_values(by=level_names[0], ascending=False)
...
cg_medpcm = F"""
<br></br>
*__Unten:__ Wanderwege Nutzen der gefundenen Objekte: Median {unit_label}.*
"""
md(cg_medpcm)
