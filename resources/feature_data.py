"""
The feature data module caries out the operations included in ver1.0
for the water body and survey area summaries. Additional features were
added. Detailed component summaries are easy to generate, a report of the
parameters is created on demand.

The default attributes are based on the parameters of the end of sampling
report in ver1.0. Once an instance of FeatureData  is instantiated
attributes can be added in dict form. Using load_defaults(FeatureData())
is the quickest way to get started.

Changing <level>, <component> and <loi> attributes changes the geographical
and the hierarchical context of the summary data. The <component> attribute
must resolve to a subset of the <level> attribute. The FeatureData class
gives the complete summary for the <level> attribute defined by the members
of <loi> and a summary of all data above <level>
"""
import pandas as pd
fd_var = {
    "sdate": "2020-03-01",
...
def load_defaults(x):
    for atts in [fd_var, loc_var, data_source]:
...
class FeatureData:
    ...
    def make_fd(self, sources: list = []):
        ...
        :type sources: list
        :return: None: results are added as attributes to the class
        instance
        """
    ...
    def summary(self, attname: str = "fd"):
        ...
        If the columns are present in the fd attribute: summarizes the
